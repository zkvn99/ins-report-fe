...
import { formatWon } from '../report/formatters.js'
...
const benchmarkPoints = computed(() => (benchmark.value?.values || []).map(item => ({ label: item.label, year: item.year, value: item.value_won })))
const benefit = computed(() => props.module.demo_benefit_id ? props.report.insurance.coverage.find(item => item.id === props.module.demo_benefit_id) : null)
...
<template>
  ...
  >
    <div class="disease-ribbon">
      <div><span>이번 페이지의 비교 질환</span><b>{{ module.disease_label }}</b><small>{{ module.disease_icd }}</small></div>
      <div class="disease-ribbon-right"><b>{{ report.customer.displayAge }}</b><span>{{ index + 1 }} / {{ total }}</span></div>
    </div>
      ...
      <div class="disease-main">
        <div class="disease-grid">
          <section class="panel">
            ...
            <p class="panel-note">{{ module.health_hint }}</p>
          </section>
          ...
          <section class="panel">
            ...
            <template v-if="benchmarkPoints.length">
              ...
              <div v-if="benefit" class="benefit-reference">
                <span>보유 {{ module.benefit_label }}</span><b>{{ formatWon(benefit.currentWon) }}</b>
              </div>
    ...
    <p class="page-note"><b>대표 질환: {{ module.disease_label }}</b>. 검진 소견이 이 질환의 진단을 뜻하지 않습니다. 공개 통계는 개인의 발병확률이나 보험금 지급 확약이 아닙니다.</p>
  </ReportPage>
</template>
