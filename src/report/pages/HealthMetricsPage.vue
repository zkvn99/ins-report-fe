...
<template>
  ...
  >
    <div class="metric-grid">
      <article v-for="metric in metrics" :key="metric.metricKey" :class="['metric-card', metric.state]">
        <div class="metric-head"><h3>{{ metric.shortName || metric.name }}</h3><StatusTag :state="metric.state" /></div>
